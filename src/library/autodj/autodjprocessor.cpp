...
void AutoDJProcessor::prepareToDeckForTransition(DeckAttributes* pToDeck) {
    VERIFY_OR_DEBUG_ASSERT(pToDeck) {
        return;
    }
    if (m_eState == ADJ_DISABLED) {
        return;
    ...
    const bool autoSync = m_pConfig->getValue(
            ConfigKey(kPreferenceGroup, QStringLiteral("AutoSyncOnTransition")),
            false);
    const bool autoKeylock = m_pConfig->getValue(
            ConfigKey(kPreferenceGroup, QStringLiteral("AutoKeylockOnTransition")),
            false);
    const mixxx::aidj::TransitionPlan plan =
            mixxx::aidj::TransitionPlanner::makePlan(autoSync, autoKeylock);
    if (plan.isNoOp()) {
        return;
    }

    // Enable sync on the playing from-deck first so it becomes soft leader;
    // then arm the to-deck as follower. Otherwise the stopped to-deck can
    // become soft leader and phase-lock to itself on play.
    DeckAttributes* pFromDeck = getOtherDeck(pToDeck);
    if (pFromDeck && plan.enableSync) {
        pFromDeck->applyTransitionPlan(plan);
    }

    if constexpr (sDebug) {
        qDebug() << this << "prepareToDeckForTransition" << pToDeck->group
                 << "sync" << plan.enableSync
...
void AutoDJProcessor::playerTrackLoaded(DeckAttributes* pDeck, TrackPointer pTrack) {
    ...
    if (duration < kMinimumTrackDurationSec) {
        ...
        // Arm sync/keylock early so the to-deck matches tempo when fade starts.
        prepareToDeckForTransition(pDeck);
